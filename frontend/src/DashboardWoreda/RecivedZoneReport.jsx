...
const ReciveZoneReport = () => {
  ...
  useEffect(() => {
    const fetchedData = async () => {
      const response = await axios.get(
        `http://localhost:5001/api/v1/woreda/${user_id}`
      );

      console.log(response.data.rows, "wewewe");
  ...
  useEffect(() => {
    axios
      .get(
        `http://localhost:5001/api/v1/report/landAdminByWoreda/${admin.woreda_id}`
      )
      .then((response) => {
...
export default ReciveZoneReport;
